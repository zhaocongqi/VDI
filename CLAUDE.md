...
- **扩展 Addon**：基于 Anaconda Addon 规范的多 Spoke 图形扩展（Python + Gtk3）

## 目录结构

```
VDI/
├── main.go              # 安装器入口 (退化为只打印 FriendlyVersion 的辅助守护 CLI)
...
```bash
make build           # 本地校验 Go 模块
make build-bundle    # 下载 RKE2 二进制/镜像/charts → package/vdi-os/iso/bundle/vdi/
make package-vdi-iso # 解包 BCLinux DVD + 注入 ks + 注入 Anaconda Addon Python 插件 + xorriso 重建
...
`feat/kickstart-xorriso` 分支已弃 elemental，改用 BCLinux 母语的 kickstart + xorriso 做"安装型"ISO（复用 BCLinux DVD 自带 anaconda stage2，非自建 squashfs Live）。构建链 `scripts/package-vdi-iso`：xorriso 解包 BCLinux DVD → 注入 `ks.cfg` + `bundle/` → 改 `isolinux.cfg`/`grub.cfg` 加 `inst.ks=hd:LABEL=BCLinux.x86_64:/ks.cfg` → xorriso 重建（`-isohybrid-mbr` 保留 BIOS+UEFI 双引导，卷标必须 `BCLinux.x86_64`）。踩坑：
...
- **`%pre` → `%include` 时序（极度重要）**：
  1. **`%include` 必须在 ks 最顶部（%pre 之前）**。`ks.cfg` 的 `%pre` 生成 `/tmp/ks-include.cfg`，`%include /tmp/ks-include.cfg` 在顶部走 anaconda 标准延迟展开。**绝不能把 %include 放 %pre 之后**——ks-include 含全局指令，落到 %pre 之后违反 kickstart "全局指令必须在所有 section 之前"语法，导致 anaconda 解析异常，且 `%post` 全段不执行。
  ...
  3. `KickstartRender` 输出基础配置段，经 %include 文本展开等价内联。
- **RKE2 离线**：`rke2.linux-amd64.tar.gz` 解压 `$SYSROOT/usr/local`（二进制内嵌 containerd，删 wharfie）；镜像 `*.tar.zst` 放 `agent/images/`，RKE2 首启自动导入（删 chroot ctr import）。`%post --nochroot` 从 `/run/install/repo/bundle/vdi` 复制 bundle 到 `$SYSROOT`，再 `$SYSROOT/etc/rancher/rke2/config.yaml` 写入 RKE2 与组件配置。
