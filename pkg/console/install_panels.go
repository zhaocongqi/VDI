...
func (c *Console) layoutInstall(_ *gocui.Gui) error {
	...
	once.Do(func() {
		...
		if alreadyInstalled {
			...
			logrus.Infof("already install value post config merge: %v", c.config.Install.Automatic)
			// if already installed and automatic installation is set to true
			// configure node directly
			if alreadyInstalled && c.config.Install.Automatic {
...
func addDiskPanel(c *Console) error {
	...
	diskFatalV.PreShow = func() error {
		diskFatalV.SetContent("No disk detected. VDI requires at least one disk.")
		return setPageTitle()
	}
	...
	wipeDisksTitlePanelV.SetContent("Additional VDI installations detected")
	...
	validateAllDiskSizes := func() (bool, error) {
		...
		if c.config.Install.SkipChecks {
			return true, nil
		}
	...
	validatePersistentPartitionSize := func(persistentSize string) (bool, error) {
		...
		if dataDisk == "" || installDisk == dataDisk {
			...
			if _, err := util.ParsePartitionSize(diskSize, persistentSize, c.config.Install.SkipChecks); err != nil {
				return false, updateValidatorMessage(err.Error())
			}
		}
		return true, nil
	...
	isWipeDisksPanelNeeded := func(g *gocui.Gui, v *gocui.View) error {
		...
		if len(options) != 0 {
			if slices.Contains(c.config.Install.WipeDisksList, c.config.Install.Device) || slices.Contains(c.config.Install.WipeDisksList, c.config.Install.DataDisk) {
				c.config.Install.WipeDisksList = []string{}
				wipeDisksV.Reset()
			}
	...
	wipeDisksConfirm := func(g *gocui.Gui, v *gocui.View) error {
		c.config.Install.WipeDisksList = wipeDisksV.GetMultiData()
		return gotoNextPage(g, v)
	}
...
func addAskCreatePanel(c *Console) error {
	askOptionsFunc := func() ([]widgets.Option, error) {
		options := []widgets.Option{
			{
				Value: config.RoleFirst,
				Text:  "Create a new VDI cluster (first node)",
			}, {
				Value: config.RoleMaster,
				Text:  "Join as a management node",
			}, {
				Value: config.RoleWorker,
				Text:  "Join as a worker node",
			},
		}
		...
		if !alreadyInstalled {
			options = append(options, widgets.Option{
				...
				Text:  "Install VDI binaries only",
			})
		}
		return options, nil
	}
	...
	askCreateV.KeyBindings = map[gocui.Key]func(*gocui.Gui, *gocui.View) error{
		gocui.KeyEnter: func(_ *gocui.Gui, _ *gocui.View) error {
			...
			if selected == config.RoleMaster || selected == config.RoleWorker {
				if alreadyInstalled {
					return showNetworkPage(c)
				}
				return showPasswordPage(c)
			}
			if selected == config.RoleFirst {
				c.config.ServerURL = ""
...
func addHostnamePanel(c *Console) error {
	...
	hostnameV.PreShow = func() error {
		...
		hostnameV.Value = c.config.OS.Hostname
...
func addNetworkPanel(c *Console) error {
	...
	preGotoNextPage := func() (string, error) {
		...
		if mgmtNetwork.Method == config.NetworkMethodDHCP {
			addr, err := getIPThroughDHCP(getManagementInterfaceName(c.config.Install.ManagementInterface))
			if err != nil {
		...
		if !c.config.Install.Silent {
			if alreadyInstalled {
				confirmV.SetContent(options +
					"\nVDI is already installed. It will be configured with the above configuration. Continue?\n")
			} else if installModeOnly {
				confirmV.SetContent(options +
					"\nVDI will be copied to local disk. No configuration will be performed. Continue?\n")
			} else {
				confirmV.SetContent(options +
					"\nYour disk will be formatted and VDI will be installed with the above configuration. Continue?\n")
			}
		}
		c.Gui.Cursor = false
...
func addConfirmUpgradePanel(c *Console) error {
	...
	confirmV.PreShow = func() error {
		return c.setContentByName(titlePanel, fmt.Sprintf("Confirm upgrading VDI to %s?", version.Version))
	}
	confirmV.KeyBindings = map[gocui.Key]func(*gocui.Gui, *gocui.View) error{
		gocui.KeyEnter: func(_ *gocui.Gui, _ *gocui.View) error {
			confirmed, err := confirmV.GetData()
...
func addInstallPanel(c *Console) error {
	...
	installV.PreShow = func() error {
		go func() {
			...
			c.config.Install.ManagementInterface.Method = strings.ToLower(c.config.Install.ManagementInterface.Method)
			c.config.Install.VipMode = strings.ToLower(c.config.Install.VipMode)
			...
			if c.config.Install.TTY == "" {
				c.config.Install.TTY = getFirstConsoleTTY()
			}
			...
			if !alreadyInstalled {
				...
				if len(preflightWarnings) > 0 {
					if c.config.Install.SkipChecks || preflightAck {
						...
						logrus.Info("Installation will proceed (vdi.install.skipchecks = true)")
					} else {
						// Checks were not explicitly skipped, fail the install
						// (this will happen when PXE booted if checks fail and
						// you don't set vdi.install.skipcheck=true)
						for _, warning := range preflightWarnings {
			...
			c.config.Install.ForceGPT = !c.config.Install.ForceMBR

			// Clear the DataDisk field if it's identical to the installation disk
			if c.config.Install.DataDisk == c.config.Install.Device {
			...
			webhooks, err := PrepareWebhooks(c.config.Install.Webhooks, getWebhookContext(c.config))
			if err != nil {
...
func addVIPPanel(c *Console) error {
	...
	gotoVipPanel := func(g *gocui.Gui, _ *gocui.View) error {
		...
		if selected == config.NetworkMethodDHCP {
			...
			spinner.Start()
			go func(g *gocui.Gui) {
				vip, err := getVipThroughDHCP(getManagementInterfaceName(c.config.Install.ManagementInterface), hwAddr)
				if err != nil {
					spinner.Stop(true, err.Error())
					g.Update(func(_ *gocui.Gui) error {
				...
				spinner.Stop(false, "")
				c.config.Install.Vip = vip.ipv4Addr
				c.config.Install.VipMode = selected
				c.config.Install.VipHwAddr = vip.hwAddr
				g.Update(func(_ *gocui.Gui) error {
			...
			c.config.Install.VipMode = config.NetworkMethodStatic
		}
	...
	gotoVerifyIP := func(g *gocui.Gui, v *gocui.View) error {
		...
		c.config.Install.Vip = vip
		c.config.Install.VipHwAddr = ""
		// gotoVipPanel is only called in DHCP mode, it is still empty in static mode
		if c.config.Install.VipMode == "" {
			c.config.Install.VipMode = config.NetworkMethodStatic
		}
		return gotoNextPage(g, v)
	}
	...
	gotoVipParentPanel := func(_ *gocui.Gui, _ *gocui.View) error {
		...
		if method == config.NetworkMethodDHCP {
			...
			return hwAddrV.SetData(c.config.Install.VipHwAddr)
		}
		return showNext(c, askVipMethodPanel)
	}
...
func configureInstallModeDHCP(c *Console) {
	...
	_, err = getIPThroughDHCP(getManagementInterfaceName(c.config.Install.ManagementInterface))
	if err != nil {
...
func checkDHCPHostname(c *config.VDIConfig, generate bool) {
	if c.OS.Hostname == "" {
		...
		if hostname != defaultHostname && hostname != "" {
			c.OS.Hostname = hostname
		} else {
			if generate {
				c.OS.Hostname = generateHostName()
			}
		}
	}
}

func mergeCloudInit(c *config.VDIConfig) error {
