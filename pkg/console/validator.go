...
var (
	...
	ErrMsgManagementInterfaceNotFound = "networks is deprecated, please use management_interface for new config"
	ErrMsgUnsupportedSchemeVersion    = "Unsupported VDI Scheme Version %d, please use new config"

	ErrContainerdRegistrySettingNotValidJSON = "could not parse containerd-registry as JSON"
)

type ValidatorInterface interface {
	Validate(cfg *config.VDIConfig) error
}
...
func checkDevice(cfg *config.VDIConfig) error {
...
func (v ConfigValidator) Validate(cfg *config.VDIConfig) error {
	// check hostname
	...
	if cfg.Install.Mode == config.ModeCreate {
		if err := checkVip(cfg.Install.Vip, cfg.Install.VipHwAddr, cfg.Install.VipMode); err != nil {
			return err
		}
	}
...
func commonCheck(cfg *config.VDIConfig) error {
	// modes
...
func validateConfig(v ValidatorInterface, cfg *config.VDIConfig) error {
...
func diskChecks(cfg *config.VDIConfig) error {
	if err := checkDevice(cfg); err != nil {
		return err
	}

	if cfg.Install.ForceMBR {
