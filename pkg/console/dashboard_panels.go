...
const (
	...
	statusSettingUpHarv = "Setting up VDI"
...
func (c *Console) getHarvesterConfig() error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			logrus.Infof("no existing VDI config detected in %s", defaultHarvesterConfig)
			return nil
		}
		return fmt.Errorf("unable to read default VDI config file %s: %v", defaultHarvesterConfig, err)
	}

	conf, err := config.LoadVDIConfig(content)
	if err != nil {
		return fmt.Errorf("failed to load VDI config from %s: %v", defaultHarvesterConfig, err)
	}
	c.config = conf
